...
(function(){
  ...
  const SALES_STORE_OVERRIDES = [
    ...
    {rede:'DIA A DIA', id:'dd_rio_verde', patterns:['GOIANIA RIO VERDE','DIA A DIA RIO VERDE']},
    {rede:'DIA A DIA', id:'dd_planaltina_df', patterns:['PLANALTINA DF']},
    {rede:'DIA A DIA', id:'dd_guara', patterns:['GUARA II','GUARA']},
...
01/04/2026;COMPER/FORT;98765,43</pre>
  ...
  function renderStatusBadge(details, scope, key){
    ...
    return `<button class="badge amber alert-click" onclick="App.showImportAlert(${jsArg(scope)},${jsArg(key)})">${details.length} alerta(s)</button>`;
  }
      ...
      ${rows.map(r=>{
        ...
        return `
          <tr class="pdf-summary-row">
            <td><button class="btn btn-sm btn-soft" onclick="App.togglePdfHistory('${escapeHtml(r.key)}')">${expanded?'−':'+'}</button></td>
            ...
            <td><button class="btn btn-sm btn-danger" onclick="App.deleteDeliveryBatch(${jsArg(r.key)})">Excluir arquivo</button></td>
          </tr>
          ${detailRows}`;
            ...
            <table class="inner-table">
              ...
              <tbody>
                ${rows.map(r=>`<tr>
                  ...
                  <td><button class="btn btn-sm btn-danger" onclick="App.deleteDeliveryImport(${jsArg(r.key)})">Excluir loja</button></td>
                </tr>`).join('')}
              </tbody>
            </table>
          </div>
  ...
  function deleteDeliveryImport(key){
    ...
    const removedRows = Store.data.deliveries.filter(d => (d.importGroupKey || `${d.date}|${d.rede}|${d.storeId}|${d.orderNumber||d.fileName||''}`) === key);
    Store.data.deliveries = Store.data.deliveries.filter(d => (d.importGroupKey || `${d.date}|${d.rede}|${d.storeId}|${d.orderNumber||d.fileName||''}`) !== key);
    Store.data.importIssues = (Store.data.importIssues || []).filter(i => i.importGroupKey !== key);
    Store.data.deletedImports ||= [];
    Store.data.deletedImports.push({id:uid('delimp'), type:'ARQUIVO_NOTA', key, removed:before-Store.data.deliveries.length, fileNames:unique(removedRows.map(d=>d.fileName)), user:state.session.usuario, createdAt:new Date().toISOString()});
    Store.save().then(()=>{toast('Loja/nota removida com sucesso.'); render();});
  }

  function deleteDeliveryBatch(batchId){
    ...
    const removedRows = Store.data.deliveries.filter(d => (d.importBatchId || `${d.fileName}|${d.rede}|${d.importedAt}`) === batchId);
    const removedIds = new Set(removedRows.map(d=>d.id));
    const removedFileNames = unique(removedRows.map(d=>d.fileName));
    const before = Store.data.deliveries.length;
    Store.data.deliveries = Store.data.deliveries.filter(d => (d.importBatchId || `${d.fileName}|${d.rede}|${d.importedAt}`) !== batchId);
    Store.data.importIssues = (Store.data.importIssues || []).filter(i => i.importBatchId !== batchId && !removedFileNames.includes(i.fileName));
    Store.data.cancelledNfes = (Store.data.cancelledNfes || []).filter(c => c.importBatchId !== batchId);
    Store.data.deletedImports ||= [];
    ...
    $('#viewRoot').innerHTML = `
      <div class="card">
        ...
        <div class="grid kpis">
          ...
          ${kpi('3','Acréscimo', '% informado', 'aplicado após a média')}
          ${kpi('4','Arredondamento', 'para cima', 'sempre número inteiro')}
        </div>
      </div>
  ...
  function renderSalesImportsTable(imports){
    return `<div class="table-wrap"><table>
      ...
      <tbody>${imports.map(i => `<tr>
        ...
        <td>${toNumber(i.unmatchedStores)+toNumber(i.unmatchedProducts) ? `<button class="badge amber" style="border:none;cursor:pointer" onclick="App.showSalesImportPendencies('${i.id}')">${fmt.format(toNumber(i.unmatchedStores)+toNumber(i.unmatchedProducts))} pendências</button>` : '<span class="badge green">OK</span>'}</td>
        <td class="num"><button class="btn btn-sm btn-danger" onclick="App.deleteSalesImport('${i.id}')">Excluir</button></td>
  ...
  async function deleteSalesImport(id){
    if (!id) return;
    Store.data.sales = (Store.data.sales || []).filter(s => s.importId !== id && s.fileId !== id);
    Store.data.salesImports = (Store.data.salesImports || []).filter(i => i.id !== id);
    Store.data.importIssues = (Store.data.importIssues || []).filter(i => i.importId !== id);
    Store.data.deletedImports ||= [];
    Store.data.deletedImports.push({id:uid('delsales'), type:'BASE_VENDA', key:id, user:state.session?.usuario || 'sistema', createdAt:new Date().toISOString()});
    await Store.save();
    toast('Base de venda removida.');
    render();
  }
