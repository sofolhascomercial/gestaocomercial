...
(function(){
  ...
  const Store = {
    ...
    scheduleCloudSave(updatedAt, delay=5000){
      if (!this.usingCloud || !this.cloud) return;
    ...
    async flushCloudSave(){
      ...
      if (this._savingCloud) {
        this.scheduleCloudSave(this._pendingCloudUpdatedAt || new Date().toISOString(), 6000);
        return;
      }
      ...
      try {
        ...
        if (this._pendingCloudUpdatedAt) this.scheduleCloudSave(this._pendingCloudUpdatedAt, 6000);
      }
    },
    ...
    async save({skipCloud=false, onProgress=null, skipSalesChunks=false}={}){
      ...
      if (this.usingCloud && !skipCloud) {
        ...
        this.scheduleCloudSave(updatedAt, shouldAvoidLocalStringify ? 7000 : 5000);
      }
    },
    async reset(){
      this.data = this.seed();
  ...
  function inferRedeFromText(text){
    ...
    if (n.includes('COSTA MULTICANAL') || n.includes('COSTA ATACADAO') || n.includes('REDE COSTA')) return 'COSTA';
    return '';
  }
  ...
  function renderOrderPage(){
    ...
    $$('#viewRoot [data-field]').forEach(inp=>{
      inp.addEventListener('input', e=>{
        ...
        Store.queueSave({}, 1800); // debounce: salva só 1,8s depois da última digitação
        renderOrderTotalsSoft(order);
      });
      inp.addEventListener('change', ()=>Store.save().then(render));
    });
    $('#orderNotes').addEventListener('input', e=>{ order.notes=e.target.value; Store.queueSave({}, 1800); });
...
01/04/2026;COMPER/FORT;98765,43</pre>
  ...
  function parsePdfPage(text, lines, rede, fileName, pageNo, batchId){
    // Extrai Fantasia com delimitadores amplos para não capturar campo seguinte
    const fantasia = (text.match(/Fantasia:\s*(.+?)(?:\s+(?:I\.E\.|Forma\s+Pgto|Data\s+Sa|Endere[cç]o|CNPJ)|\n|$)/i)||[])[1]?.trim() || '';
    // Extrai CNPJ/CPF do PDF (ex: 27.289.076/0016-11)
    const cnpjRaw = (text.match(/CNPJ\/CPF\.?:\s*([\d.\/\-]+)/i)||[])[1]?.replace(/[^\d]/g,'') || '';
    const dataSaida = parseDate((text.match(/Data Sa[ií]da\.?:\s*(\d{1,2}\/\d{1,2}\/\d{4})/i)||[])[1]);
    // Normaliza número do pedido usando apenas a parte antes da barra (evita chaves de duplicidade divergentes)
    const pedidoRaw = (text.match(/PEDIDO\s*\n?\s*([0-9]+(?:\s*\/\s*[0-9]+)?)/i)||[])[1] || '';
    const pedido = pedidoRaw.split('/')[0].replace(/\s/g,'') || uid('pedido');
    ...
    // Resolve rede a partir do texto da página (ex: COSTA MULTICANAL S/A -> COSTA)
    const redeInferida = rede || inferRedeFromText(text);
    // Tenta resolver loja pelo CNPJ primeiro (mais preciso), depois pelo nome fantasia
    const store = (cnpjRaw ? matchStoreByCnpj(cnpjRaw, redeInferida) : null) || matchStore(fantasia, redeInferida);
    const unmatched = [];
    const records = [];
    const pageText = lines.join(' ');
    const foundItems = extractPdfItems(pageText);
    const noteFound = !!fantasia || foundItems.length > 0 || /PEDIDO/i.test(text);
    const importGroupKey = `${dataSaida || todayISO()}|${store?.rede || redeInferida || 'REDE?'}|${store?.id || fantasia || 'LOJA?'}|${pedido}`;
